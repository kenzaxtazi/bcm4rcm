...
# Custom libraries
# Custom libraries
DIR = json.load(open("../../config.json"))["code_dir"]  # noqa
sys.path.append(DIR)  # noqa
from load import aphrodite
from bcm4rcm.experiments.validation.metrics import compute_crps
...
# Get weights
if weighting == "alpha":
    weight_path = DIR + "bcm4rcm/data/weights/250107_weights_beta_no_p95_5x5.npy"
    print(weight_path)
    alpha_arr = np.load(weight_path)

if weighting == "alpha_p95":
    weight_path = DIR + "bcm4rcm/data/weights/250105_weights.npy"
    print(weight_path)
    alpha_arr = np.load(weight_path)
...
if weighting == "alpha_p95_scaling":
    ...
    alpha_arr = np.load(weight_path)
    aphro_p95 = np.load(
        "/data/hpcdata/users/kenzi22/bcm4rcm/data/weights/archive/p95/p95_aphro_1951_1980.npy"
    )
    p95_path = (
        DIR + "bcm4rcm/data/weights/archive/p95/p95_historical_*" + ref_year + ".npy"
    )
    print(p95_path)
...
for j in tqdm.tqdm(range(len(bcm_files))):
    ...
    moe_samples = [
        np.random.normal(m, s, int(np.ceil(100 * w)))
        for m, s, w in zip(mu, std, weights)
    ]
    moe_samp = [
    ...
    # Generate EW samples
    ew_samples = [
        np.random.normal(m, s, int(np.ceil(100 / 13))) for m, s in zip(mu, std)
    ]
    ew_samp = [np.nan_to_num(sp.special.inv_boxcox(ms, lmbda), 0) for ms in ew_samples]
    ew_sample_list.append(ew_samp)

moe_list = [
    np.concatenate([moe_sample_list[j][i] for j in range(13)])[:100]
    for i in range(12 * 180 * 80)
]
ew_list = [
    np.concatenate([ew_sample_list[j][i] for j in range(13)])[:100]
    for i in range(12 * 180 * 80)
]
...
moe_df.to_csv(
    DIR
    + "bcm4rcm/data/moe_outputs/rcm_crps_"
    + weighting
    + "_"
    + target_experiment
    + "_"
