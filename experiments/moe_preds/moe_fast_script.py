...
# Get weights
if weighting == 'alpha':
    weight_path = directory + 'bcm4rcm/data/weights/250110_weights_beta_p95_for_wass_no_p95_for_dist_5x5_1951_2005.npy'
    print(weight_path)
...
for j in tqdm.tqdm(range(len(bcm_files))):
    ...
    moe_samples = [np.random.normal(m, s, int(np.ceil(n*w))) for m, s, w in zip(mu, std, weights)]
...
# MoE statistics
moe_list = [np.concatenate([moe_sample_list[j][i] for j in range(13)])[:n] for i in range(12*180*80)]
...
# EW statistics
ew_list = [np.concatenate([ew_sample_list[j][i] for j in range(13)])[:n] for i in range(12*180*80)]
...
# Save statistics
moe_df.to_csv(directory + 'bcm4rcm/data/moe_outputs/moe_' + weighting + '_' + target_experiment + '_' + 
